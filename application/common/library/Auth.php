...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score','agency_id','score_rmb','level','level_info'];
     ...
     * @return boolean
     */
    public function register($username, $password, $email = '', $mobile = '', $extend = [],$group_id=2)
    {
        // 检测用户名或邮箱、手机号是否存在
        ...
        //同一手机号、统一角色只允许一个用户存在
        if ($mobile && model('User')->where('group_id',$group_id)->getByMobile($mobile))
        {
    ...
    public function getUserinfo()
    {
        ...
        if ($data['agency_id']){
            $check=Agency::get($data['agency_id']);
            if ($check){
                $userinfo['agency']=$check->toArray();
            }else{
                $userinfo['agency']=(object)[];
            }
            $agency_member=model('AgencyMember')->where(['agency_id'=>$data['agency_id'],'uid'=>$data['id'],'status'=>1])->find();
            $userinfo['agency']=(array)$userinfo['agency'];
            if (!empty($agency_member)){
                $userinfo['agency']['type_text']=$agency_member['type_text'];
            }else{
                $userinfo['agency']['type_text']="";
            }
        }else{
            $userinfo['agency']=(object)[];
        }
        $userinfo['join_agency_list']=model('Agency')->get_my_join($data['id']);
        $userinfo['token'] = $this->getToken();
        if (!empty($userinfo['agency']) && !empty($userinfo['agency']['type_text']) && $userinfo['agency']['creator']==$userinfo['id']){
            $group=UserGroup::get(4);
            $group['rules_list']=UserRule::get_user_rule([],4);
            $userinfo['group']=$group;
        }else{
            $userinfo['group']=$this->_user->group;
        }
        $userinfo['share_url']=config('img_domain').url('api/invite/user_share_index',['code'=>base64_encode($userinfo['id'])]);
        return $userinfo;
    }
