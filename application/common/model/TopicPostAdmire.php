...
class TopicPostAdmire extends Model{
    protected $name='topic_post_admire';
    ...
    {
       if ($cid==0){
           \model('TopicPost')->where(['id'=>$pid])->setInc('likecount');
       }else{
           \model('TopicPostComment')->where(['pid'=>$pid,'id'=>$cid])->setInc('likecount');
       }
        $ip=request()->ip(0);
        $res=GetIpLookup($ip);
    ...
    public static function cancel_admire($pid,$cid=0,$uid)
    {
        ...
        if ($info){
            if ($cid==0){
                \model('TopicPost')->where(['id'=>$pid])->setDec('likecount');
            }else{
                \model('TopicPostComment')->where(['pid'=>$pid,'id'=>$cid])->setDec('likecount');
            }
        }
        return $info;
    }
