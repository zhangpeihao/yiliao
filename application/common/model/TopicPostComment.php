...
class TopicPostComment extends Model{
    protected $name='topic_post_comment';
    ...
    {
        TopicPostComment::event('after_insert',function($comment){
            model('TopicPost')->where('id',$comment['pid'])->setInc('pcounts');
            if ($comment['bid']>0){
                TopicPostComment::where('id',$comment['bid'])->setInc('pcounts');
            }
        });
    }
    ...
    {
        if ($data['bid']){
            $parent_comment=\model('topic_post_comment')->where('id',$data['bid'])->find();
