...
/**
 * 琴友圈评论
 * Class TopicPostComment
 * @package app\api\controller
 */
class TopicPostComment extends Api{
    ...
    public function get_list()
    {
        ...
        $uid=$this->auth->id;
        $data=model('TopicPostComment')->where($map)->order('id desc')->paginate($page_size,[],['page'=>$page])
            ->each(function ($val,$key)use ($uid){
                $val['is_admire']=TopicPostAdmire::is_admire($val['pid'],$val['id'],$uid);
                return $val;
            })
            ->jsonSerialize();
...
//        $pbid=$this->request->post('pbid',0,'intval');
        ...
        $info=\app\common\model\TopicPostComment::create([
            'pid'=>$pid,'pbid'=>$pbid,'bid'=>$bid,'buid'=>$buid,'content'=>$content,'pics'=>$pics,'uid'=>$this->auth->id,
            'ip'=>$ip,'locate'=>$locate[1].$locate[2]
        ]);
        if ($info){
            if ($bid==0){
                model('TopicPost')->where('id',$pid)->setInc('pcounts');
            }else{
                model('TopicPostComment')->where('id',$bid)->setInc('pcounts');
            }
            $this->success('添加成功');
        }else{
            $this->error('操作失败');
    ...
    private function get_pbid($bid,$tmp_id=0){
        $check=db('topic_post_comment')->where('id',$bid)->find();
        if ($check['pbid']){
