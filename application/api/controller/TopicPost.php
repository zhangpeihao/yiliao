...
class TopicPost extends Api{
    ...
    public function get_list()
    {
        ...
        $data=model('TopicPost')->where($map)->order('id desc')->paginate($page_size,'',['page'=>$page])
                ->each(function ($val,$key)use ($uid){
                    if ($uid){
                        $val['is_admire']=TopicPostAdmire::is_admire($val['id'],0,$uid);
                    }else{
    ...
    public function get_detail()
    {
        ...
        $data=model('TopicPost')->where('id',$id)->find();
        $data['is_admire']=0;
        if (empty($this->auth->id)){$this->auth->id=0;}
        $data['is_admire']=TopicPostAdmire::is_admire($id,0,$this->auth->id);
        $data['share_page']=request()->domain().url('api/share/topic_page',['id'=>$id]);
...
//                dump($result);exit();
        ...
        $topic_post=new \app\common\model\TopicPost();
        $info=$topic_post->data($data)->save();
        if ($info){
            //视频传上服务器，服务器做队列转换视频编码
            if ($video){
                VideoLog::add_video_task('topic_post',['id'=>$topic_post->id],'video',$data['video']);
            }
            $this->success('发布成功');
        }else{
            $this->error('发布失败');
    ...
    public function add_admire()
    {
        ...
        if (TopicPostAdmire::is_admire($pid,$cid,$uid)){
            $info=TopicPostAdmire::cancel_admire($pid,$cid,$uid);
            if ($info){
                $this->success('取消点赞成功');
            }else{
                $this->error('已经点赞过了');
            ...
            $info=TopicPostAdmire::add_admire($pid,$cid,$uid);
            if ($info){
                $this->success('点赞成功');
            }else{
                $this->error('操作失败');
    ...
    public function delete_post()
    {
        ...
        $check=db('topic_post')->where('id',$id)->where('status',1)->find();
        if ($check){
            if ($check['uid']!=$this->auth->id){
                $this->error('无删除权限');
            }
            $info=\app\common\model\TopicPost::update(['status'=>0],['id'=>$check['id']]);
            if ($info){
                $this->success('删除成功');
            }else{
                $this->error('删除失败');
    ...
    public function edit_post()
    {
        ...
        $info=\app\common\model\TopicPost::update($data,['id'=>$id]);
        if ($info){
            $this->success('发布成功');
        }else{
            $this->error('发布失败');
